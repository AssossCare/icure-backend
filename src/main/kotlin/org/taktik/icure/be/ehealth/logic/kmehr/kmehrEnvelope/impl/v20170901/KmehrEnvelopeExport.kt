...
@org.springframework.stereotype.Service("kmehrEnvelopeExport")
class KmehrEnvelopeExport : KmehrExport() {
    override val log = LogFactory.getLog(KmehrEnvelopeExport::class.java)
    ...
    fun createKmehrEnvelope(
        ...
        fillPatientFolder(folder, pat, sfks, sender, language, config, note, tags, contexts, isPsy, documentId, transactionType, decryptor)
    ...
    internal fun fillPatientFolder(folder: FolderType, p: Patient, sfks: List<String>, sender: HealthcareParty, language: String, config: Config, note: String?, tags: List<String>, contexts: List<String>, isPsy: Boolean, documentId: String?, transactionType: String?, decryptor: AsyncDecrypt?): FolderType {
        val trn = TransactionType().apply {
            cds.add(CDTRANSACTION().apply { s(CDTRANSACTIONschemes.CD_TRANSACTION); value = transactionType })
